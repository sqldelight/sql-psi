...
class SampleHeadlessParser {
  fun parseSqlite(sourceFolders: List<Path>, onError: (String) -> Unit): List<SampleFile> {
    ...
    environment.annotate { element, message ->
      val file =
        PsiDocumentManager.getInstance(environment.project).getDocument(element.containingFile)!!
      val error =
        "${element.containingFile.virtualFile.path}: (${file.getLineNumber(element.textOffset) + 1}, ${
        element.textOffset - file.getLineStartOffset(file.getLineNumber(element.textOffset))
      }): $message"
      onError(error)
    }
    return buildList { environment.forSourceFiles<SampleFile> { add(it) } }
  }
}
